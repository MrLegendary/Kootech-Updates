...
ICONPLAYLISTS  = uservar.ICONPLAYLISTS if not uservar.ICONPLAYLISTS == 'http://' else ICON
...
def index():
	...
	addDir ('Playlists'     ,'playlists',icon=ICONPLAYLISTS, themeit=THEME1)
	addDir ('Onderhoud'   ,'maint',    icon=ICONMAINT,     themeit=THEME1)
	if (wiz.platform() == 'android'): addDir ('Apk Installer' ,'apk',      icon=ICONAPK,      themeit=THEME1)
	if HIDECONTACT == 'No': addDir('Contact'      ,'info',  icon=ICONCONTACT,  themeit=THEME1)
	if HIDESPACERS == 'No': addFile('-----------------', '', themeit=THEME3)
	addDir('Instellingen'      ,'settings', icon=ICONSETTINGS,  themeit=THEME1)
...
def viewBuild(name):
	...
	for version, url, gui, kodi, themefile, icon, fanart in match:
		...
		if BUILDNAME == name and version > BUILDVERSION:
			build = '%s [COLOR red][B][v%s GEINSTALLEERD][/B][/COLOR]' % (build, BUILDVERSION)
		addFile(build, '', fanart=fanart, icon=icon, themeit=THEME2)
		addFile('==========[ Installeren ]==========', '', fanart=fanart, icon=icon, themeit=THEME3)
		addFile('Schone Installatie (aanbevolen)'   , 'install', name, 'fresh'   , fanart=fanart, icon=icon, themeit=THEME1)
		addFile('Standaard Installatie', 'install', name, 'normal'  , fanart=fanart, icon=icon, themeit=THEME1)
...
def maintMenu():
	...
	addFile('Cache verwijderen'     		,'clearcache',      icon=ICONMAINT, themeit=THEME1)
	addFile('Packages verwijderen'      	,'clearpackages',   icon=ICONMAINT, themeit=THEME1)
	addFile('Thumbnails verwijderen'    	,'clearthumb',      icon=ICONMAINT, themeit=THEME1)
	addFile('Databases verwijderen'     	,'purgedb',         icon=ICONMAINT, themeit=THEME1)
	addDir ('Addons verwijderen'        	,'removeaddons',    icon=ICONMAINT, themeit=THEME1)
	addDir ('Addon data verwijderen'    	,'removeaddondata', icon=ICONMAINT, themeit=THEME1)
	addFile('Forceer Addon Updates'  		,'forceupdate',     icon=ICONMAINT, themeit=THEME1)
	addFile('Kodi geforceerd afsluiten'     ,'forceclose',      icon=ICONMAINT, themeit=THEME1)
	addFile('===[ Automatisch Schoonmaken ]===', '', fanart=FANART, icon=ICONMAINT, themeit=THEME3)
	addFile('Automatisch schoonmaken bij opstarten: %s' % autoclean.replace('true',on).replace('false',off) ,'togglesetting', 'autoclean',   icon=ICONMAINT, themeit=THEME1)
	if autoclean == 'true':
		addFile('Cache verwijderen bij opstarten: %s' % cache.replace('true',on).replace('false',off) ,'togglesetting', 'clearcache', icon=ICONMAINT, themeit=THEME1)
		addFile('Packages verwijderen bij opstarten: %s' % packages.replace('true',on).replace('false',off) ,'togglesetting', 'clearpackages', icon=ICONMAINT, themeit=THEME1)
	setView('files', 'MAIN')
...
def buildWizard(name, type, theme=None):
	if type == 'gui':
		...
		if KODIV < 16.0 and float(wiz.checkBuild(name, 'kodi')) >= 16.0:
			...
			yes_pressed = DIALOG.yesno(ADDONTITLE, 'Wilt u %s v%s installeren?' % (name, wiz.checkBuild(name,'version')), nolabel='Nee',yeslabel='Ja')
		if yes_pressed:
			...
			if not os.path.exists(PACKAGES): os.makedirs(PACKAGES)
			DP.create(ADDONTITLE,'%s aan het downloaden...' % name,'', 'Even Geduld...')
			...
			DIALOG.ok(ADDONTITLE, "Om de wijzigingen door te voeren dient Kodi te worden afgesloten, druk op OK om dit nu te doen.")
			wiz.killxbmc()
		else:
...
def checkUpdate():
	vcheck = wiz.checkBuild(BUILDNAME,'version')
...
def launchPlaylists():
	...
	if os.path.exists(playlist):
		...
		if not os.path.exists(PACKAGES): os.makedirs(PACKAGES)
		lib=os.path.join(PACKAGES, playlistzip)
		try: os.remove(lib)
		except: pass
...
def freshStart(install=None):
	...
	if SKIN not in ['skin.confluence']:
		...
		yes=DIALOG.yesno(ADDONTITLE, "Uw skin moet eerst worden teruggezet naar [COLOR blue]%s[/COLOR]" % skin[5:], "Wilt u dit nu doen?", nolabel="Nee", yeslabel="Ja");
		if yes:	
		...
		else: wiz.LogNotify(ADDONTITLE,'Fresh Install: [COLOR red]Geannuleerd![/COLOR]'); return
	if install: yes_pressed=DIALOG.yesno(ADDONTITLE,"Wilt u uw Kodi configuratie terugzetten naar de","standaardinstellingen voordat %s geinstalleerd wordt?" % install, "[COLOR green](aanbevolen)[/COLOR]", nolabel='Nee', yeslabel='Ja')
	...
	if yes_pressed:
		...
		DP.close()
		if install: 
			DIALOG.ok(ADDONTITLE, "Uw huidige installatie is verwijderd!", "Nu wordt %s v%s geinstalleerd!" % (install, wiz.checkBuild(install,'version')))
			buildWizard(install, 'normal')
		else:
			DIALOG.ok(ADDONTITLE, "Klaar!, u hebt nu een schone installatie met alleen Kootech Updates","Herstart uw systeem om de wijzigingen door te voeren")
			wiz.killxbmc()
	else: wiz.LogNotify(ADDONTITLE,'Fresh Install: [COLOR red]Geannuleerd![/COLOR]'); xbmc.executebuiltin('Container.Refresh')
...
elif mode=='checkupdate'    : checkUpdate()
...
elif mode=='info'           : wiz.TextBoxes(ADDONTITLE, INFO)
elif mode=='playlists'      : launchPlaylists()
...
if mode not in ['', 'togglesettings', 'settings', 'info']: xbmcplugin.endOfDirectory(int(sys.argv[1]))